...
import { ensureAndSimulateDiabetesData } from "../../services/simulationService";
...
const PatientDiabetesPrediction = () => {
    ...
    const loadData = useCallback(async () => {
        try {
            setLoading(true);
            const latestRes = await getDiabetesLatestPrediction(patientId);
            const historyRes = await getDiabetesPredictionHistory(patientId);
    ...
    useEffect(() => {
        if (patientId) {
            // Initial check & baseline simulation trigger
            (async () => {
                await ensureAndSimulateDiabetesData(patientId);
                await loadData();
            })();

            // Automated 15-second simulation loop matching SIMULATION-SERVICE cycle
            const interval = setInterval(async () => {
                try {
                    await ensureAndSimulateDiabetesData(patientId);
                    await loadData();
                } catch (err) {
                    console.warn("Automated 15s diabetes simulation update note:", err);
                }
            }, 15000);
    ...
    const handlePredictAgain = async () => {
        ...
        try {
            ...
            // 1. Ensure simulated diabetes data is updated in MongoDB
            await ensureAndSimulateDiabetesData(patientId);

            // 2. Trigger AI prediction model
            await predictDiabetesPatient(patientId);

            await loadData();

            const latest = await getDiabetesLatestPrediction(patientId);
            if (latest?.data?.id) {
...
export default PatientDiabetesPrediction;
